begin;
create function public.ops_state(p_token text) returns jsonb
...
begin
 ...
 'drivers',(select coalesce(jsonb_agg(jsonb_build_object('id',d.id,'name',d.name,'phone',d.phone)),'[]') from public.drivers d where d.org_id=actor.org_id and (actor.role<>'driver' or d.id=actor.actor_id)),
...
create function public.ops_create_session(p_org uuid,p_actor uuid,p_role text,p_token_hash text) returns void
...
create function public.ops_proof_uploaded(p_token text,p_proof uuid,p_hash text) returns void
language plpgsql security definer set search_path=pg_catalog,operations as $$
...
create function public.ops_revoke_session(p_token text) returns void
language plpgsql security definer set search_path=pg_catalog,operations as $$
