begin;
create unique index one_driver_in_transit on operations.routes(driver_id) where status='in_transit';
create function operations.refresh_route(p_route uuid) returns void
...
create function public.ops_command(p_token text,p_id uuid,p_kind text,p_payload jsonb default '{}'::jsonb) returns jsonb
language plpgsql security definer set search_path=pg_catalog,operations,public as $$
declare
...
begin
 ...
 when 'geocode' then
  ...
  if p_payload->>'lat' is not null and p_payload->>'lng' is not null and (p_payload->>'lat')::double precision between -90 and 90 and (p_payload->>'lng')::double precision between -180 and 180 then
   update operations.manifest_rows set raw=raw||jsonb_build_object('coords',jsonb_build_object('lat',(p_payload->>'lat')::double precision,'lng',(p_payload->>'lng')::double precision)),error=null where id=mr.id and error is null;
 ...
 when 'assign' then
  ...
  if target_driver is not null and not exists(select 1 from public.drivers where id=target_driver and org_id=actor.org_id) then raise exception 'Driver is not a member of this organization'; end if;
 ...
 when 'claim' then
  ...
  if not exists(select 1 from public.drivers where id=actor.actor_id and org_id=actor.org_id) then raise exception 'Driver organization mismatch'; end if;
